...
public class IntArrayQueue {
    ...
    //peak() : 큐애소 대아토룰 피크(맨앞 데이터를 들여다 본다)
    public int peak() throws EmptyIntArrayQueueException{
        if(num <= 0) throw new EmptyIntArrayQueueException();
        return que[num-1];
    }
    ...
    //큐에서 x 를 검색하여 발견하지 못하면 -1 을 반환하는 indexOf() 기능구현
    public int indexOf(int x){
        for(int  i = 0; i < num; i ++){
            if(que[i] == x) return i; // 검색 성공시 해당 인덱스 전달
        }
        return -1; // 검색 실패
    }
